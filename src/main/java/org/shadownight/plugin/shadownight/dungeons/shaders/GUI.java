...
public class GUI {
    ...
    public static void main() {
        ...
        final int PX_PER_GRID   = 20;       // Pixels per unit square in Perlin grid
        final boolean SHOW_GRID = false;  // Select grid visibility
        final long SEED         = 0;           // Seed to shuffle the hash table
        ...
        for (int x = 0; x < SCR_WIDTH; x++) {
            for (int y = 0; y < SCR_HEIGHT; y++) {
                ...
                if (!SHOW_GRID || (x % PX_PER_GRID != 0 && y % PX_PER_GRID != 0))
                    val = PerlinNoise.perlinCalc(xInGrid, yInGrid);
